...
window.revertToRound = async (r) => {
    ...
    const data = snap.val();
    if (data.host === myName && confirm(`Rewind game to Round ${r}?`)) {
...
function syncApp(snap) {
    ...
    if (data.status === "waiting") {
        ...
        document.getElementById('leaderboard').innerHTML = sorted.map(p => `<div class="p-row"><b>${p.name} ${p.submitted ? '✅' : '⏳'}</b><span>${p.total} pts</span></div>`).join("");
        ...
        for (let r = data.roundNum; r >= 1; r--) {
            let rows = playersArr.map(p => `<div class="history-row"><span>${p.name}</span><b>${p.history[r] || 0}</b></div>`).join("");
